...
In particular, every element of $F(a)$ can be written uniquely as $\sum_{i<d}c_i a^i$ for some $(c_i:i<d)\in F^d$.
...
Since $b$ is a root of $\tau(p)$, we have $(p)\subseteq\ker g_b$.
As $p$ is irreducible, $(p)$ is maximal; moreover, $\ker g_b$ is
proper because $g_b(1)=1\neq 0$. Hence $\ker g_b=(p)$, and there is
...
and $\sigma_{\sigma(a)}=\sigma$, since these homomorphisms agree on $K$ and $a$.
Thus $g(f(\sigma))=\sigma$.
\end{proof}
...
\begin{proof}
       ...
       Since $p$ is irreducible over $F$ and $p(a_j)=0$, it is also the minimal polynomial of $a_j$ over $F$.
       Moreover, as $m_j>1$ and $F$ has characteristic zero, $1\leq\deg p'=\deg p-1<\deg p$, contradicting its minimality.
\end{proof}
