...
In this section, we work in $\ZFm$, except in Theorem~\ref{thm:algebraic}, where we also assume $\MC$.
...
Suppose for a contradiction that for every $\alpha<\kappa$ we have $F_{\alpha+1}\ne F_\alpha$.
Then $(F_\alpha)_{\alpha<\kappa}$ is a strictly increasing chain of subfields of $L$ of length $\kappa$, yielding an injection of $\kappa$ into $\Pow(L)$, contrary to the definition of $\kappa$.
